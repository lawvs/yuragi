...
rg -n \
  "buildShardKeyframes|planShardTimings" \
  packages/core/dist/**/*.d.ts
```

Expected: neither command finds a match. The private `buildShardKeyframes()` and
`planShardTimings()` helpers remain absent from the declarations generated by
the preceding `pnpm build`. Historical mentions in changelogs and this
design/implementation plan are intentional.
